...
export class SqliteAdapter {
    ...
    async find(id) {
        const row = findStmt.get(this.model, id, Math.floor(Date.now() / 1000));
        return rowToPayload(row);
    }
    async findByUserCode(userCode) {
        const row = findByUserCodeStmt.get(this.model, userCode, Math.floor(Date.now() / 1000));
        return rowToPayload(row);
    }
    async findByUid(uid) {
        const row = findByUidStmt.get(this.model, uid, Math.floor(Date.now() / 1000));
        return rowToPayload(row);
    }
    async consume(id) {
