...
export const getPodLogsTool = {
    ...
    execute: async ({ namespace, podSearch }) => {
        try {
            // 1. Discovery
            const res = namespace
            ...
            if (limitedPods.length === 0) {
                return { content: [{ type: "text", text: "No pods found matching criteria." }] };
            }
            // 4. Retrieve Logs
            const results = {};
            for (const pod of limitedPods) {
                ...
                try {
                    ...
                    results[`${podNamespace}/${podName}`] = logs.body;
                }
                catch (e) {
...
export const countPodsTool = {
    ...
    execute: async ({ namespace }) => {
        try {
            ...
            return {
                content: [{ type: "text", text: `Total pods: ${pods.length}` }],
            };
        }
        catch (e) {
            console.error("Kubernetes API Error:", e.response?.data || e.message);
